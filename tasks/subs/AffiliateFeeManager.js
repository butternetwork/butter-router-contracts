...
task("AffiliateFeeManager:deploy", "deploy AffiliateFeeManager")
...
task("AffiliateFeeManager:triggleRegisterWhitelist", "triggleRegisterWhitelist")
    .setAction(async (taskArgs, hre) => {
        const { network, ethers } = hre;
        const accounts = await ethers.getSigners();
        const deployer = accounts[0];
...
task("AffiliateFeeManager:updateWhitelist", "updateWhitelist")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        let affiliateFeeManager_address = getDeployment(network.name, "AffiliateFeeManager");
        let affiliateFeeManager = AffiliateFeeManager.attach(affiliateFeeManager_address);
        console.log(`address ${taskArgs.wallet} pre whitelist status :`, await affiliateFeeManager.whitelist(taskArgs.wallet));
        await (await affiliateFeeManager.updateWhitelist(taskArgs.wallet, taskArgs.flag)).wait()
        console.log(`address ${taskArgs.wallet} after whitelist status :`, await affiliateFeeManager.whitelist(taskArgs.wallet));
    });
...
task("AffiliateFeeManager:grantRole", "grantRole")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        console.log(`${taskArgs.user} has ${taskArgs.role} role`, await affiliateFeeManager.hasRole(role, taskArgs.user));
    });

task("AffiliateFeeManager:upgrade", "deploy butterRouterV2")
    .setAction(async (taskArgs, hre) => {
        const { network, ethers } = hre;
        const accounts = await ethers.getSigners();
        const deployer = accounts[0];
