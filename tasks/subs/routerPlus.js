...
let {routerPlus,
    ...
    tronSetFee,
    tronSetAuthFromConfig} = require("../utils/tronPlus.js")
...
module.exports = async (taskArgs,hre) => {
        ...
        if(network.name === 'Tron' || network.name === 'TronTest'){
            await routerPlus(hre.artifacts,network.name,config);
        } else { 
            console.log("\ndeployer :", deployer)
...
task("routerPlus:deploy", "deploy butter router plus")
    ...
    .setAction(async (taskArgs,hre) => {
        ...
        const { deployer } = await getNamedAccounts();
        if(network.name === 'Tron' || network.name === 'TronTest'){
           await deployRouterPlus(hre.artifacts,network.name,taskArgs.wtoken);
        } else { 
...
task("routerPlus:setFee", "set fee ")
    ...
    .setAction(async (taskArgs,hre) => {
        ...
        const { deployer } = await getNamedAccounts();
        if(network.name === 'Tron' || network.name === 'TronTest'){
            await tronSetFee(hre.artifacts,network.name,taskArgs.router,taskArgs.feereceiver,taskArgs.feerate,taskArgs.fixedfee);
        } else {
