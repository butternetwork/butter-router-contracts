...
describe("FeeReceiver", function () {
    ...
    it("editConverter -> revert Ownable: caller is not the owner", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(shares1).editConverter(shares1.address, true)).to.be.revertedWith(
    ...
    it("addStablecoins -> revert Ownable: caller is not the owner", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(shares1).addStablecoins(token1.address)).to.be.revertedWith(
            "Ownable: caller is not the owner"
        );
    });
    ...
    it("addStablecoins -> correct", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(owner).addStablecoins(token1.address)).to.be.emit(
            feeReceiver,
            "AddStablecoin"
        );
    ...
    it("editConverter -> correct", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(owner).editConverter(shares1.address, true)).to.be.emit(
            feeReceiver,
            "EditConverter"
        );
    ...
    it("release -> revert account has no shares", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(owner).release(token1.address, owner.address)).to.be.revertedWith(
    ...
    it("release -> revert unsuport release token", async () => {
        await deployFixture();
        await expect(feeReceiver.connect(shares1).release(token1.address, owner.address)).to.be.revertedWith(
            "account has no shares"
        );
    });
    ...
    it("release -> correct", async () => {
        ...
        await expect(feeReceiver.connect(shares1).release(token1.address, shares1.address)).to.be.emit(
            feeReceiver,
            "PaymentReleased"
        );
    ...
    it("convertToStablecoin -> convert deny", async () => {
        ...
        let convertParam = {
            ...
            playload: Convert.interface.encodeFunctionData("convert", [
                token1.address,
                token2.address,
        ...
        await expect(feeReceiver.connect(shares1).convertToStablecoin([convertParam])).to.be.revertedWith(
            "convert deny"
        );
    });
    ...
    it("convertToStablecoin -> correct", async () => {
        ...
        await expect(feeReceiver.connect(owner).convertToStablecoin([convertParam])).to.be.emit(
            feeReceiver,
            "ConvertTo"
        );
