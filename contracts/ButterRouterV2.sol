...
contract ButterRouterV1{
        ...
        (_token,_amount,_mapTargetToken,_toChain,_toAddress,_exchangeData,_butterCore,_mosAddress) = abi.decode(bridgeParams,
         ...
         if(_token == address(0)){
            ...
            }else if(_exchangeData.input_Out_Addre[1] == address(0)){
                TransferHelper.safeTransferFrom(_token,msg.sender,address(this),_amount);
                swapOutNatives(_amount,_mapTargetToken,_toChain,_toAddress,_exchangeData,_butterCore,_mosAddress);
            } else {
                TransferHelper.safeTransferFrom(_token,msg.sender,address(this),_amount);
                swapOutTokens(_token,_amount,_mapTargetToken,_toChain,_toAddress,_exchangeData,_butterCore,_mosAddress);
        ...
        function swapOutTokens(address _token,uint256 _amount,address _mapTargetToken,uint256 _toChain,bytes memory _toAddress,ExchangeData memory _exchangeData,address _butterCore,address _mosAddress) internal{
          ...
          function swapOutNatives(uint256 _amount,address _mapTargetToken,uint256 _toChain,bytes memory _toAddress,ExchangeData memory _exchangeData,address _butterCore,address _mosAddress) internal{
         ...
         function swapOutTokenToToken(address _token,uint256 _amount,address _mapTargetToken,uint256 _toChain,bytes memory _toAddress,ExchangeData memory _exchangeData,address _butterCore,address _mosAddress) internal{
