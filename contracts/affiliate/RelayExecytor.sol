...
contract RelayExecytor is AccessControlEnumerable, IRelayExecutor {
    ...
    function _exeute(
      ...
      returns (
        ...
        if(len != 0) {
            try feeManager.collectAffiliatesFee(_orderId, _token, _amount, _message[offset: (offset += len * 4)]) returns (uint256 totalFee) {
